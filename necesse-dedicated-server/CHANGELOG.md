...
- Clarify Network UDP port help text: this is the port players type in Necesse; change it only when 14159 is already taken (e.g. a second Necesse app)
...
- Remove HA `backup_interval_minutes` and `server_port` options (parallel/confusing knobs)
  ...
  - Network UDP port is what players use; the game listens inside the app and Home Assistant forwards
