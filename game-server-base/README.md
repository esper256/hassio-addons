...
6. Bump versions, then install via the HA repo or Docker compose modeled on an existing game.
   ...
   **Version scheme:** `{supervisor_major}.{supervisor_minor}.{game_patch}`.
   ...
   (`SUPERVISOR_VERSION`, currently `3.4`). Each game `config.yaml` is that
   plus a patch (`3.4.0` for the first release on supervisor 3.4).

   - Supervisor change → bump `SUPERVISOR_VERSION` (e.g. `3.3` → `3.4`) **and**
     set **every** game add-on to `{new}.0` (`3.4.0`) so users see them all
     update together.
   - Game-only fix (no supervisor change) → bump that game’s patch only
     (`3.4.0` → `3.4.1`). Leave other games and `SUPERVISOR_VERSION` alone.
...
The supervisor writes a stable Linux machine-id to `/data/supervisor/machine-id` and copies it to `/etc/machine-id` (and dbus / DMI product_uuid when those paths are writable). HAOS/Docker images often have none; Java and other binaries use it as the hardware UUID.
