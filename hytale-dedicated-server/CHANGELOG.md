...
- Shared supervisor **3.4**: empty player / version-mismatch pattern notes log only when the live tailer starts, so the Ingress 5-second status poll no longer repeats them. Every add-on persists a Linux machine-id under `/data/supervisor` and copies it to `/etc/machine-id` (and dbus / DMI when writable) so binaries that look for a hardware UUID work in HAOS/Docker. Vendored `game_server/` sync.
