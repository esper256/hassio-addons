...
5. Open **Open Web UI**. Complete both sign-in cards (new browser tab, not the Ingress iframe). The card code is the device login; if Hytale emails a code, use the email code in that field, not the card code.
...
- Sign-in card while the downloader or `/auth login device` is waiting (open the URL in a **new tab**; do not paste the card code into an email-login box)
