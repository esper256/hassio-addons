#!/usr/bin/env bash
# Thin HA entrypoint: point at the Factorio plugin and start the generic supervisor.
set -euo pipefail
...
# is fixed at 34197 in config.yaml. Factorio must listen on that same container
...
# Blank HA server_name → stable "HAOS Factorio ####" (per-install salt under /data).
