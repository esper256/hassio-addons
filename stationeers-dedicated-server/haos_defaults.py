"""Stationeers-only helpers for Home Assistant / Docker defaults.
...
SERVER_NAME_PREFIX = "HAOS Stationeers"
...
def default_server_name(
    *,
    ...
    """Build ``HAOS Stationeers ####`` from a persisted install salt."""

    root = Path(state_dir or os.environ.get("STATE_DIR") or "/data/supervisor")
    salt = ensure_instance_salt(root / INSTANCE_SALT_NAME)
    digest = hashlib.sha256(f"stationeers-server-name|{salt}".encode("utf-8")).hexdigest()
